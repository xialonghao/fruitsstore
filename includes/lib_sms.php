...
function sendsms_imtong($mobile, $content)
{
	...
	$sendurl = "http://www.smsbao.com/sms?u=".$GLOBALS['_CFG']['ecsdxt_user_name']."&p=".md5($GLOBALS['_CFG']['ecsdxt_pass_word'])."&m=".$mobile."&c=".urlencode($content)."";
...
{
	return (strlen($mobile) == 11 || strlen($mobile) == 12) && (preg_match("/^13\d{9}$/", $mobile) || preg_match("/^14\d{9}$/", $mobile) || preg_match("/^15\d{9}$/", $mobile) || preg_match("/^18\d{9}$/", $mobile) || preg_match("/^17\d{9}$/", $mobile) || preg_match("/^0\d{10}$/", $mobile) || preg_match("/^0\d{11}$/", $mobile));
}
