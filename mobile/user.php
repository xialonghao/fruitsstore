...

{
	...
	/*O菜龙工作室（www.ocailong.com）新增显示分销会员标准*/
	...
	if($rank_points>$level_register_up||$rank_points==$level_register_up){
		...
		$xkfla="掌柜：否     <a href='./'>点此链接购买成为掌柜</a>";
	...
	if($type=='weixin')
		{
			...
			$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".WX_AKEY."&redirect_uri=".urlencode($config['site_url'].'mobile/')."weixin_login.php&response_type=code&scope=snsapi_userinfo&state=".md5(mt_rand(1,100))."#wechat_redirect";
		}
		else
		{
...

{
	...
	require(dirname(__FILE__) . '/include/lib_sms.php');
...
elseif ($action == 'order_tracking')
{
    ...
    //生成快递100查询接口链接
    $query_link = kuaidi100($orders['shipping_name'] , $orders['invoice_no']);
    //优先使用curl模式发送数据
...

    {
...

        {
...

            {
                ...
                }elseif ($is_wechat==false && $payment['pay_code'] == 'wx_new_jspay'){
					unset($payment_list[$key]);
				}
...

{
    ...
    if (affirm_received($order_id, $user_id))
    {
		...
		if ($_CFG['ecsdxt_customer_confirm'] == '1' && $row['mobile_phone'] != '')
		{
			...
			$ret = sendsms($mobilephone, $content);
		}
...

{
...

    {
		...
		//开始开发，兼容微信支付-模板之家-www.ocailong.com
        ...
        $sql = 'UPDATE ' . $GLOBALS['ecs']->table('pay_log') .
               ...
               " SET user_account_id = order_id , order_id = '$new_order_id' WHERE log_id = ".$order['log_id'];
...

{
...

	{
	   ...
	   $sql = "select * from " . $ecs->table('user_card') . " where card_no='$card_no' and card_pass='$card_pass' ";
...

{
	...
	//O菜龙工作室（www.ocailong.com）显示会员等级和下面人数
...

{
	...
	foreach($user_list['user_list'] as $key =>$value){
		...
		if($value['level']==1){
			...
			$value['head_url']=$GLOBALS['db']->getOne("SELECT  headimgurl FROM " . $ecs->table('weixin_user')." WHERE wxid = '$wxid'");

			$value['nickname']=$GLOBALS['db']->getOne("SELECT nickname FROM " . $ecs->table('weixin_user')." WHERE wxid = '$wxid'");
...
function kuaidi100($shipping_name,$invoice_sn){
