...
*O菜龙工作室（www.ocailong.com）分销微分销分销中心
...
if ($action == 'default')
{
	...
	/*O菜龙工作室（www.ocailong.com）分销微分销新增显示分销会员标准*/
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	...
	$res[5]=$count5;
	//print_r($res);
	$smarty->assign('xkfla',        $xkfla);
	/*O菜龙工作室（www.ocailong.com）分销微分销新增显示分销会员标准*/
	$smarty->assign('service_phone',        $_CFG['service_phone']);
	/*O菜龙工作室（www.ocailong.com）分销微分销新增查询佣金*/
	//推荐注册分成
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$user_list['user_list'] = array();
	$auid = $_SESSION['user_id'];
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
			...
			foreach($user_info as $key=>$value){
				/*O菜龙工作室（www.ocailong.com）分销微分销开发未付款的佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount,sum(goods_amount) as goods_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=0";
				...
				$xkflaarr['weifukuan']['order_num']+=$order_info['order_num'];
				$xkflaarr['weifukuan']['order_amount']+=$order_info['order_amount'];
				$xkflaarr['weifukuan']['goods_amount']+=$order_info['goods_amount'];
				$xkflaarr['weifukuan']['setmoney']+=$setmoney;
				/*O菜龙工作室（www.ocailong.com）分销微分销开发已经付款且未收货佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount,goods_amount,sum(goods_amount) as goods_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=2 and   shipping_status  <> 2";
				$order_info=$db->getRow($sql);
				$k=$i-1;
				$setmoney = round($order_info['order_amount'] * $affiliate['item'][$k]['level_money'], 2);
				$xkflaarr['yifukuan']['order_num']+=$order_info['order_num'];
				$xkflaarr['yifukuan']['order_amount']+=$order_info['order_amount'];
				$xkflaarr['yifukuan']['goods_amount']+=$order_info['goods_amount'];
				$xkflaarr['yifukuan']['setmoney']+=$setmoney;
				/*O菜龙工作室（www.ocailong.com）分销微分销开发已经收货佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount,goods_amount,sum(goods_amount) as goods_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']."  and  shipping_status  =2";
				$order_info=$db->getRow($sql);
				$k=$i-1;
				$setmoney = round($order_info['order_amount'] * $affiliate['item'][$k]['level_money'], 2);
				$xkflaarr['yishouhuo']['order_num']+=$order_info['order_num'];
				$xkflaarr['yishouhuo']['order_amount']+=$order_info['order_amount'];
				$xkflaarr['yishouhuo']['goods_amount']+=$order_info['goods_amount'];
				$xkflaarr['yishouhuo']['setmoney']+=$setmoney;				
			}
			foreach($xkflaarr as $key=>$vale){
				if($key=="weifukuan"){
					$allarr[$key]['order_num']+=$vale['order_num'];
					$allarr[$key]['order_amount']+=$vale['order_amount'];
					$allarr[$key]['goods_amount']+=$vale['goods_amount'];
	...
	foreach($allarr  as $key =>$value){
		$xkfla2015all['order_num']+=$value['order_num'];
		$xkfla2015all['order_amount']+=$value['order_amount'];
		$xkfla2015all['goods_amount']+=$value['goods_amount'];
		$xkfla2015all['setmoney']+=$value['setmoney'];
	}
	$smarty->assign('xkfla2015all',        $xkfla2015all);
	$smarty->assign('xkfla2015arr',        $allarr);
	/*O菜龙工作室（www.ocailong.com）分销微分销新增查询佣金*/
	...
	$smarty->assign('surplus_amount',       get_user_surplus($_SESSION['user_id']));
...
/*O菜龙工作室（www.ocailong.com）分销微分销开发*/
...

{
    ...
    $smarty->display('user_passport_xkfla2015.dwt');
...

{
	...
	/*O菜龙工作室（www.ocailong.com）分销微分销优化提现流程*/
	...
	$xkfla2015="真实姓名:【".$real_name."】开户行:【".$bank_name."】银行账户:【".$bank_account."】手机:【".$mobile_phone."】留言:【".$user_note."】";
...

    {
        ...
        $pay_obj = new $payment_info['pay_code'];
				...
				//开始O菜龙工作室（www.ocailong.com）分销开发，兼容微信支付
...

{
	...
	//O菜龙工作室（www.ocailong.com）分销显示会员等级和下面人数
...

{
	...
	foreach($user_list['user_list'] as $key =>$value){
		...
		if($value['level']==1){
			...
			$value['head_url']=$GLOBALS['db']->getOne("SELECT  headimgurl FROM ". $GLOBALS['ecs']->table('weixin_user') ." WHERE wxid = '$wxid'");

			$value['nickname']=$GLOBALS['db']->getOne("SELECT nickname FROM ". $GLOBALS['ecs']->table('weixin_user') ." WHERE wxid = '$wxid'");
...

    {
...

        {
			...
			foreach($user_info as $key=>$value){
				...
				/*O菜龙工作室（www.ocailong.com）修复 */

				$xkfla_level_money=$affiliate['item'][$k]['level_money'];

				$setmoney = round($order_info['order_amount'] * $xkfla_level_money, 2);
...
//O菜龙工作室（www.ocailong.com）分销微分销开发结束
	...
	foreach($logdb  as $key=>$value){
		...
		//print_r($value);
			...
			if($value['is_separate']==1){
				...
				$if_xkfla2015=$value['user_id']==$user_id&&$value['suid']==$_SESSION['user_id'];

			}else{

				$if_xkfla2015=$value['user_id']==$user_id;
