	...
	if(empty($fromUsername)){
		...
		$msg = "非法操作";
			...
			if($type==1){
				...
				$jf_state = $db -> getOne("SELECT `autoload` FROM ". $GLOBALS['ecs']->table('weixin_point') ." WHERE `point_name` = 'share_dfpoint'");

				if ($jf_state == 'yes'){

					$info="分享给朋友返积分".$qd_jf;

					$qd_jf = $db -> getOne("SELECT `point_value` FROM ". $GLOBALS['ecs']->table('weixin_point') ." WHERE `point_name` = 'share_dfpoint'");
			...
			elseif($type==2){
				...
				$jf_state = $db -> getOne("SELECT `autoload` FROM ". $GLOBALS['ecs']->table('weixin_point') ." WHERE `point_name` = 'share_fpoint'");

				if ($jf_state == 'yes'){

					$info="分享朋友圈返积分".$qd_jf;

					$qd_jf = $db -> getOne("SELECT `point_value` FROM ". $GLOBALS['ecs']->table('weixin_point') ." WHERE `point_name` = 'share_fpoint'");
