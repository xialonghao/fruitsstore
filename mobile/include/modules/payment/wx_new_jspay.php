...
class wx_new_jspay
{
    function __construct()
    {
        ...
        if(!defined('WXAPPID'))
        {
            ...
            define('WXNOTIFY_URL',$GLOBALS['ecs']->url().'wx_native_callback.php');
    ...
    function get_code($order, $payment)
    {
        ...
        if (!isset($_GET['code']))
        {
            ...
            $pay_url = $jsApi->createOauthUrlForCode($redirect);
        }else
        {
            $code = $_GET['code'];
            $jsApi->setCode($code);
        ...
        if($openid)
        {
            ...
            $html .= '</script>';
			$html .= '<div style="text-align:center"><input type="button" onclick="callpay()" value="微信支付" class="c-btn3" /></div>';
        ...
        else
        {
            ...
            $html .= "}";
            $html .= '</script>';
			$html .= '<div style="text-align:center"><input type="button" onclick="window.location.href=\''.$pay_url.'\'"  value="微信支付" class="c-btn4" /></div>';
