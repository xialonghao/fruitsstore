...
class AlipaySubmit {
     ...
     * @return 提交表单HTML文本
     */
	function buildRequestForm($para_temp, $method, $button_name, $alipay_key) {
		...
		$sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_gateway_new."_input_charset=".trim(strtolower($this->alipay_config['input_charset']))."' method='POST'>";
		while (list ($key, $val) = each ($para)) {
            $sHtml.= "<input type='hidden' name='".$key."' value='".$val."'/>";
        }

		//submit按钮控件请不要含有name属性
        $sHtml = $sHtml."<div style='text-align:center'><a href='".$this->alipay_gateway_new.$param."&sign=".md5($sign)."'
                   class='J-btn-submit c-btn3'>支付宝付款</a></div></form>";
        $sHtml = $sHtml.<<<eot
