...
elseif($_REQUEST['act'] == 'add') 
{
	...
	if($_POST)
	{
		...
		$json_arr = array('action_name'=>$action_name,'action_info'=>array('scene'=>array('scene_id'=>$affiliate)));
...
elseif($_REQUEST['act'] == 'edit') 
{
	...
	if($_POST)
	{
		...
		if($ticket)
		{
			...
			$update_sql = "UPDATE  ". $GLOBALS['ecs']->table('weixin_qr') ." SET  `scene` =  '$scene',`ticket` =  '$ticket',`function` = '$function',`qr_path` = '$qr_path',`affiliate` = '$affiliate',`endtime` = '$endtime',`dateline` = '$dateline' WHERE  `qid` ='$id';";
			$db->query($update_sql);
			$link[] = array('href' =>'wxch-ent.php?act=tuijian', 'text' => $wxch_lang['ur_here']);
			sys_msg('修改成功',0,$link);
		}
