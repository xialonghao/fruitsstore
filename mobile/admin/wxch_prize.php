...
if($_REQUEST['act'] == 'add') 
{
	...
	if($_POST) 
	{
		...
		{
			$update_sql = "UPDATE ". $GLOBALS['ecs']->table('weixin_prize_append') ." SET `prize_name` = '$prize_name[$k]',`prize_value` = '$prize_value[$k]' WHERE `id` =$prize_id[$k] AND `prize_id` = '$pid';";
			$db->query($update_sql);
		}
...
elseif($_REQUEST['act'] == 'news') 
{
	...
	if($_POST) 
	{
		...
		{
			$insert_sql = "INSERT INTO ". $GLOBALS['ecs']->table('weixin_prize_append') ." (`prize_id`, `prize_name`, `prize_value`) VALUES ('$post_pid', '$prize_name[$k]', '$prize_value[$k]')";
			$db->query($insert_sql);
		}
		$url = 'wxch_prize.php?act=edit&pid='.$pid;
		$link[] = array('href' =>$url, 'text' => $wxch_lang['ur_here']);
...
elseif($_REQUEST['act'] == 'add_prize') 
{
	...
	if($_POST) 
	{
		...
		$time = time();
		$insert_sql = "INSERT INTO ". $GLOBALS['ecs']->table('weixin_prize') ." (`title`, `fun`, `num`, `count`, `loop`, `starttime`, `endtime`, `dateline`) VALUES
...
elseif($_REQUEST['act'] == 'edit_prize') 
{
	...
	if($_POST) 
	{
		...
		$endtime = strtotime($_POST['endtime']);
		$time = time();
		$insert_sql = "UPDATE ". $GLOBALS['ecs']->table('weixin_prize') ." SET `title` = '$name',`fun` = '$fun',`point`='$point',`loop`= '$loop',`num` = '$num',`starttime` = '$starttime',`endtime` = '$endtime',`dateline` = '$time' WHERE `pid` ='$pid';";
		$db->query($insert_sql);
	...
	{
		$sql = "SELECT * FROM ". $GLOBALS['ecs']->table('weixin_prize') ." WHERE `pid` = $pid";
		$ret = $db->getRow($sql);
