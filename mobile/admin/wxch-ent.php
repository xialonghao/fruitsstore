...
if($_REQUEST['act'] == 'wxconfig') 
{
	...
	if(!empty($_POST['token']))
	{
		...
		$ret = $db->query("UPDATE ". $GLOBALS['ecs']->table('weixin_config') ." SET `token`='$token',`appid`='$appid',`appsecret`='$appsecret' WHERE `id`=1;");
...
{	
	...
	foreach($ret as $k)
	{
		...
		{
			$db->query("DELETE FROM ". $GLOBALS['ecs']->table('weixin_keywords') ." WHERE ". $GLOBALS['ecs']->table('weixin_keywords') .".`id` = $k[id]");
		}
	}
