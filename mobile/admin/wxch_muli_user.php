...
elseif($_REQUEST['act'] == 'add') 
{
	...
	if($_POST)
	{
		...
		$insert_sql = "INSERT INTO ". $GLOBALS['ecs']->table('weixin_muli_user') ." (`name`,`token`,`appid`, `appsecret`, `gh_id` ,`user_id`,`user_name`,`datetime`) VALUES
...
elseif($_REQUEST['act'] == 'edit') 
{
	...
	if($_POST)
	{
		...
		$update_sql = "UPDATE  ". $GLOBALS['ecs']->table('weixin_muli_user') ." SET  `name` =  '$name',`appid` = '$appid',`appsecret` = '$appsecret',`gh_id` = '$gh_id',`user_id` = '$user_id',`datetime` = '$datetime',`user_name` = '$user_name' WHERE  `id` ='$id';";
	...
	$id = $_GET['id'];
	$data = $db->getRow("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_muli_user') ." WHERE `id` = $id");
	$lang = array();
...
elseif($_REQUEST['act'] == 'remove'){
		...
		$db->query("DELETE FROM  ". $GLOBALS['ecs']->table('weixin_muli_user') ." WHERE  id = '$id' ");
...
elseif($_REQUEST['act'] == 'diymen'){
	...
	}else
	{
			...
			{
				sys_msg('生成菜单失败，请重新生成一次，如多次仍然不行，请联系：O菜龙工作室',0,$link);
			}
	}
...
elseif($_REQUEST['act'] == 'query') 
{
	if(!empty($_POST['keyword']))
	{
		...
		$filter['start'] = $start;
		$ret = $db->getAll("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_qr') ." WHERE `name` LIKE '%$keyword%' LIMIT $start , $filter[page_size]");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
			if($v['type'] == 1)
...
{
	access_token($db,$id);
	$ret = $db->getRow("SELECT `access_token` FROM ". $GLOBALS['ecs']->table('weixin_muli_user') ." where id='$id'");
...
{
	$time = time();
	$ret = $db->getRow("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_muli_user') ." WHERE `id` = '$id'");
	$appid = $ret['appid'];
	...
	if($ret->access_token)
	{
		$db->query("UPDATE ". $GLOBALS['ecs']->table('weixin_muli_user') ." SET `access_token` = '$ret->access_token',`dateline` = '$time' WHERE `id` ='$id';");
	}
