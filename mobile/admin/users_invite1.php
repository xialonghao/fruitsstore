...
/*         $sql = "SELECT eu.user_id,wu.affiliate, eu.user_name, eu.email, eu.is_validated, eu.user_money, eu.frozen_money, eu.rank_points, eu.pay_points, eu.reg_time FROM " . $GLOBALS['ecs']->table('users')."  eu,wxch_user wu where (wu.wxid = eu.wxid )ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
			...
			$sql = "SELECT eu.user_id, eu.user_name, eu.email, eu.is_validated, eu.user_money, eu.frozen_money, eu.rank_points, eu.pay_points, eu.reg_time FROM ". $GLOBALS['ecs']->table('users') ."  eu  ,". $GLOBALS['ecs']->table('weixin_qr') ." wq where ( wq.affiliate = eu.user_id) ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size']; 
