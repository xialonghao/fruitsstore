...
elseif($_REQUEST['act'] == 'add') 
{
	...
	if($_POST)
	{
		...
		$ret = $db->getRow("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_qr') ." WHERE `action_name` = 'QR_LIMIT_SCENE' ORDER BY `scene_id` DESC");
		...
		if($ticket)
		{
			...
			$insert_sql = "INSERT INTO ". $GLOBALS['ecs']->table('weixin_qr') ." (`type`,`expire_seconds`,`action_name`,`ticket`, `scene_id`, `scene` ,`qr_path`,`function`) VALUES
...
elseif($_REQUEST['act'] == 'edit') 
{
	if($_POST)
	{
		...
		$expire_seconds = $_POST['expire_seconds'];
		$function = $_POST['function'];
		$ticket = $db->getOne("SELECT `ticket` FROM ". $GLOBALS['ecs']->table('weixin_qr') ." WHERE `qid` = '$id' ");
		if($ticket)
		{
			$ticket_url = urlencode($ticket);
			$ticket_url = 'https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='.$ticket_url;
			...
			$update_sql = "UPDATE  ". $GLOBALS['ecs']->table('weixin_qr') ." SET  `scene` =  '$scene',`function` = '$function',`expire_seconds` = '$expire_seconds',`qr_path` = '$qr_path' WHERE  `qid` ='$id';";
			$db->query($update_sql);
			$link[] = array('href' =>'wxch-ent.php?act=qr', 'text' => $wxch_lang['ur_here']);
			sys_msg('修改成功',0,$link);
		}
	...
	$id = $_GET['id'];
	$data = $db->getRow("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_qr') ." WHERE `qid` = $id");
	$lang = array();
...
elseif($_REQUEST['act'] == 'remove') 
{
	...
	$filter['start'] = $start;
	$ret = $db->getAll("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_qr') ." LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
