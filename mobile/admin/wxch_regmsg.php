...
elseif($_REQUEST['act'] == 'add') 
{
	if($_POST)
	{
		...
		{
		$keyword="关注回复文本";
		$sql = "SELECT id FROM  ". $GLOBALS['ecs']->table('weixin_keywords1') ." where type=3";
		$id=$db->getOne($sql);
		$type=3;
		$contents = strip_tags($_POST['contents'],"<a>");
		if($id){
			$update_sql = "UPDATE  ". $GLOBALS['ecs']->table('weixin_keywords1') ." SET  `name` =  '$name',`keyword` =  '$keyword',`type` =  '$type',`name` =  '$name',`contents` =  '$contents' WHERE  `id` ='$id';";
		$db->query($update_sql);
		}else{
			$contents = htmltowei($contents);
			$db->query("INSERT INTO ". $GLOBALS['ecs']->table('weixin_keywords1') ." (`name`, `keyword`, `type`, `contents`, `count`, `status`) VALUES
		...
		elseif($type == 2)
		{
			$sql = "SELECT id FROM  ". $GLOBALS['ecs']->table('weixin_keywords1') ." where type=4";
			$id=$db->getOne($sql);
			if(empty($id)){
			...
			if(!empty($kws_id))
			{
				$db->query("UPDATE  ". $GLOBALS['ecs']->table('weixin_keywords1') ." SET  `name` =  '$name',`keyword` = '$keyword',`type` = '$type',`status` =  '1' WHERE `id` =$kws_id;");
			}
		}else{
			$name ="关注回复图文";
			$keyword = "关注回复图文";
	...
	if(empty($id)){
		...
		elseif($_GET['type'] == 'image')
		{
			...
			$data = $db->getRow("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_keywords1') ." WHERE `id` = $id");
			if($_GET['type'] == 'image') 
			{
...
elseif($_REQUEST['act'] == 'edit') 
{
	if($_POST)
	{
		...
		$type = $_POST['type'];
		$contents = strip_tags($_POST['contents'],"<a>");
		$update_sql = "UPDATE  ". $GLOBALS['ecs']->table('weixin_keywords') ." SET  `name` =  '$name',`keyword` =  '$keyword',`type` =  '$type',`name` =  '$name',`contents` =  '$contents' WHERE  `id` ='$id';";
		$db->query($update_sql);
	...
	$id = $_GET['id'];
	$data = $db->getRow("SELECT * FROM ". $GLOBALS['ecs']->table('weixin_keywords') ." WHERE `id` = $id");
	if($_GET['type'] == 'image') 
	{
		$article_list = get_keywords_articles($id, $db);
	}
...
elseif ($_REQUEST['act'] == 'add_article') 
{
	...
	{
		$insert_sql = "INSERT INTO ". $GLOBALS['ecs']->table('weixin_keywords') ." (`name`) VALUES
('');";
		$db->query($insert_sql);
		$kws_id = $db->insert_id();
	...
	{
		$sql = "INSERT INTO ". $GLOBALS['ecs']->table('weixin_keywords_article') ." (kws_id, article_id) VALUES ('$kws_id', '$val')";
		$db->query($sql);
	}
...
elseif ($_REQUEST['act'] == 'drop_article') 
{
	...
	foreach ($articles AS $val) 
	{
		$sql = "DELETE FROM ". $GLOBALS['ecs']->table('weixin_keywords_article') ." WHERE `article_id` = $val;";
		$db->query($sql);
	}
