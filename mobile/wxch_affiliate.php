...
if($action == 'affiliate')
{
    ...
    $m_ret = $db->getOne("SELECT `cfg_value` FROM  ". $GLOBALS['ecs']->table('weixin_cfg') ." WHERE `cfg_name` = 'murl'");
    $base_ret = $db->getOne("SELECT `cfg_value` FROM  ". $GLOBALS['ecs']->table('weixin_cfg') ." WHERE `cfg_name` = 'baseurl'");
