...
$back_url = $db->getOne("SELECT `contents` FROM ". $GLOBALS['ecs']->table('weixin_oauth') ." WHERE `oid` = '$oid'");
$affiliate_id = $db->getOne("SELECT `affiliate` FROM ". $GLOBALS['ecs']->table('weixin_user') ." WHERE `wxid` = '$openid'");
...
//模板之家修复完善
$update_sql = "UPDATE ". $GLOBALS['ecs']->table('weixin_oauth') ." SET `count` = `count` + 1 WHERE `oid` = $oid; ";
$db->query($update_sql);
