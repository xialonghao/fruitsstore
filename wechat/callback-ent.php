...
( '$fromUsername', '$keyword', $time);");
			...
			if (strlen($ret_22['user_name']) == 28) {
				$sql_del = "UPDATE ". $GLOBALS['ecs']->table('users') ." SET `user_rank` = '$rank_id',`wxch_bd`='no' WHERE `wxid` ='$fromUsername'";
				$db -> query($sql_del);
			} 
			$zhanghaoinfo="";
			if (empty($uname)) {
				...
				if (empty($wxch_user_wxid)) {
					if (empty($wxch_user_name)) {
						...
						if (empty($wxch_nobd_wxid)) {
							...
							if ($uc_state) {
								...
								$wxch_user_sql = "INSERT INTO ". $GLOBALS['ecs']->table('users') ." (`user_id`,`user_name`,`password`,`wxid`,`user_rank`,`wxch_bd`) VALUES ('$ecs_user_id','$uc_username','$ecs_password','$fromUsername','$rank_id','no')";
								$db -> query($wxch_user_sql);
							} else {
							
								$autoreg_state = $db -> getOne("SELECT `state` FROM ". $GLOBALS['ecs']->table('weixin_autoreg') ." WHERE `autoreg_id` = 1");
								if($autoreg_state){
									$wxch_user_sql = "INSERT INTO ". $GLOBALS['ecs']->table('users') ." ( `user_name`,`password`,`wxid`,`user_rank`,`wxch_bd`) VALUES ('$fromUsername','$ec_pwd','$fromUsername','$rank_id','no')";
									$db -> query($wxch_user_sql);
									$ecs_user_id = $db -> insert_id();
					...
					$wxch_user_sql = " UPDATE ". $GLOBALS['ecs']->table('users') ." SET `user_rank` = '$rank_id',`wxch_bd`='no' WHERE `wxid` ='$fromUsername'";
					$db -> query($wxch_user_sql);
				} 
			}
...
( '$fromUsername', '$menu_message', $time);");
			...
			if ($keyword == 'debug') {
				...
				$imgs = $name;
				$target = '../qrcode/xkfla2015.jpg';//背景图片
			...
			}if($keyword == 'ewm')
			{
				...
				$image6='../images/qrcode/headimg_6401';
			...
			$headimgurl = '../images/qrcode/headimg_640/'.$fromUsername.'.jpg';//www.ocailong.com
		...
		if($ticket)
		{
			...
			if(false !==$local_file){
				...
				$imgs = $name;
				$target = 'qrcode/ocailong_com.jpg';//背景图片
				$target_img = Imagecreatefromjpeg($target);
			...
			//O菜龙工作室（www.ocailong.com）修复绑定上下级关系		
			...
			if ($aff_arr[0] == 'affiliate') {
				if (!empty($aff_arr[2])) {
					...
					if(empty($parent_id)){
						if (!empty($aff_db)&&$flag&&$flagexkfla2015) {
							//绑定会员账号
							...
							/*O菜龙工作室（www.ocailong.com）新增扫描关注带提醒*/
							$up_uid=$aff_arr[2];
...
nation=大雁塔&mode=driving&region=西安';
	...
	protected function plusPoint($db, $uname, $keyword, $fromUsername) {
		...
		//O菜龙工作室（www.ocailong.com）修复积分赠送次数限制
